...
$$\beta_1 \sim N(0,1/10,000)$$
$$\beta_2 \sim N(0,1/10,000)$$
...
model{
  ...
  b1 ~ dnorm(b0,1/10000)  
  b2 ~ dnorm(b0,1/10000)    ## multivariate Normal prior on vector of regression params
...
plot(jags.out)
```
The trace plots for b1 and b2 appear to have converged since the chains are overlapping. The trace of tau does have overlap, but not as strong as b1 and b2, indicating that it may not have converged yet.
...
cor(out) 
```
b1 and b2 appear to be uncorrelated. The correlation values associated with tau are small, but the graphs appear different than the typical correlation plots.
