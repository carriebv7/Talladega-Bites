...
# Tick Data
<<<<<<< HEAD
We loaded the tick-count dataset for Talledaga (monthly_tick_temp.csv), which was manually parsed from the raw data to extract monthly counts. The total observations are 58. 
=======
(manually parsed)

>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
...
<<<<<<< HEAD
SC: just in case I mess something up, I'm adding this stuff separately.

Process Model:
We represent the true monthly tick abundance (N) with a discrete-time logistic-growth process plust stochastic noise. At each month i, the population evoves according to:
$$
\begin{aligned}
N_{\mathrm{ticks}}[i] 
...
Here, Tau[i] is the elapsed time between observations, ensuring the model accomodates any irregular spacing. This formulation captures density dependence, time-step scaling growth, and random fluctuations around the deterministic trend.

Data Model:
We link the latent abundance to our observed counts y[i] through a Poisson observation model:
$$
y[i]\sim pois(N_{\mathrm{ticks}})
$$
This assumes that, conditional on the true mean of N_ticks[i], the observed number of ticks in month i arises from a Poisson process and ensures our tick count is not negative.

#Data formatting for JAGS input
We convert the month.year strings into proper Date objects (first of each month), compute the month‐to‐month intervals to scale the process noise (TimeStep), round the tick counts to integers for the Poisson model, and then bundle the key components—tick counts, number of observations, and time steps into a list for JAGS.
=======
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
...
<<<<<<< HEAD
We then implement the above model in JAGS, specifying priors for K, r, Sigma, and the latent initial state, looping through the process‐model update with multiplicative time‐step noise, and linking to the data via a Poisson likelihood.
=======
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
log_solution_model <- "
...
<<<<<<< HEAD
And here we have the actual model running step. We're drawing from 150,000 iterations per chain to ensure convergence and discarding the first 10,000 as burn-in to reduce dependence on initial values. Convergence diagnostics are assessed in the following sections. 
=======
...
We visually inspected convergence and stationarity by plotting trace and density plots for the key parameters (r, K, Sigma), three latent states (N[5], N[10], N[15]), and three process‐noise draws (E[5], E[10], E[15]). Well‐mixed, “hairy caterpillar” trace plots with no apparent trends and smooth, unimodal density curves indicate that the chains have likely converged and explored the posterior distributions adequately.
=======
...
We computed potential‐scale‐reduction factors for both key parameters (r, K, sigma) and representative latent‐state and noise draws (N[5], N[10], N[15], E[5], E[10], E[15]). In the Gelman plots, the median and 97.5%‐quantile shrink factors quickly fall to ~1 and remain stable throughout the chains. The tabulated PSRF point estimates and upper 97.5% credible limits are all ≤ 1.01, with a multivariate PSRF of exactly 1, indicating that all chains have mixed and converged successfully.
=======
# Diagnostics
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
...
-Process-noise precision (sigma) is small (∼0.0018), implying modest month-to-moth stochastic variability.
=======
# Posterior summary statistics
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
...
-A slight negative correlation between r and K, reflecting the usual growth‐rate vs. carrying‐capacity trade‐off.
-A positive correlations among the latent states (N[5], N[10], N[15]), since each month’s abundance builds on the previous.
-Little to no correlation between the process‐noise draws (E[i]) and the deterministic parameters (r, K, sigma).
=======
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
```{r}
...
# Time Series
<<<<<<< HEAD
The figure below shows the posterior median estimate of monthly tick count (blue line) with its 95% credible interval (blue shading) compared to the observed tick counts (white-filled circles). The model closely racks the seasonal peaks and troughs in the data. The credible intervals widen slightly where variability is greatest, which is expected as we enter stretches with missing data (e.g., over winter months). The uncertainty accumulates and the ribbon grows accordlingly.
=======
...
## Helper Functions
<<<<<<< HEAD
Now we streamline our forecasting and plotting workflow with three helper functions. First, forecastN generates an ensemble of n trajectories over steps months; at each time step it computes the Beverton–Holt projection, applies pmax(0, μ_t) to enforce non‐negativity, and advances the state. Second, calc_forecast_ci takes the resulting n × steps forecast matrix and returns both the column‐wise 2.5%/97.5% quantiles and the medians, facilitating quick access to credible intervals and central estimates. Finally, draw_env draws a shaded polygon on a time‐series plot by linking the lower and upper quantile vectors in ci with the corresponding dates, using the specified color and no border. Together, these functions let us efficiently simulate, summarize, and visualize forecast uncertainty in a few concise lines of code.
=======
...
<<<<<<< HEAD
Let's start by partitioning out the initial condition uncertainty.To isolate the contribution of uncertainty in the final latent‐state estimate alone, we sampled 1,000 draws of N[58] (the last inferred month’s abundance) from the posterior, while holding the growth rate r and carrying capacity fixed at their posterior means and omitting process noise. We then used our forecastN() function to propagate only these initial‐condition variations forward for 12 months. The resulting grey ribbon in the zoomed plot shows the 95% interval arising solely from uncertainty in the starting state, and the black line marks its median forecast under this scenario. This envelope quantifies how much of the total forecast spread is attributable purely to ambiguity in the most recent observed abundance.
=======
...
Now we sample 1,000 draws of N[58], r, and K from the posterior, still omitting process noise. We then forecast each draw deterministically for 12 months using our forecastN() helper. The red envelope (N.IP) captures the combined uncertainty from both the initial state and the parameters. For comparison, the grey ribbon shows the initial‐condition‐only uncertainty from the previous step.Visually, the red envelope is substantially wider than the grey one, indicating that parameter uncertainty contributes more to total forecast variance than initial‐condition ambiguity. This tells us that, although knowing the exact current tick count matters, refining our estimates of the growth rate and carrying capacity would yield the largest reduction in near‐term forecast error.
=======
## Initial Ccondition uncertainty + Parameter uncertainty
...
<<<<<<< HEAD
Next we extend our uncertainty partiion by including the stochastic process noise term. We redefine forecastN() to draw each monthly abundance from a normal distribution with mean given by the Beverton–Holt projection and standard deviation, rather than a deterministic pmax(0, μ). Sampling 1,000 draws of N[58], parameters(r, K), and noise precision (σ), we propagate each trajectory forward with process noise. The resulting dark‐blue envelope (N.IPDE) shows the 95% CIs when both initial‐condition, parameter, and process‐noise uncertainties are included. Overlaid in lighter red and gray are the previous two envelopes (N.IP and N.I), which show how each additional source of uncertainty incrementally widens our forecast intervals.
=======
...
<<<<<<< HEAD
Since we don't have any random effect in the model, we're going to add some noise to our sigma term to simulate that. We draw 1,000 posterior samples of N[58], growth rate, carrying capacity, and noise precision, then multiply each noise precision by a random factor that is uniformly sampled from 0.8-1.2. using our forecastN() helper (which now injects process noise according to these perturbed noise precision values), we generate a new forecast matrix and compute its 95% CIs. The resulting orange ribbon (N.IPDEA) sits almost entirely within the dark‐blue process‐noise envelope (N.IPDE), indicating that this extra random‐effect variance contributes only marginally to total forecast uncertainty (much less so than the core process‐noise and parameter components).
=======
...
he black layer (initial‐condition) remains present throughout but decays from its peak contribution, falling close to zero by around April.
...
<<<<<<< HEAD
#Conlcusion
In conclusion, our time series model successfully captured the seasonal dynamics of tick count at Talladega and provided a framework for forecasting and uncertainty analysis. The process model reproduced historical peaks and troughs, while the Poisson data model ensured non-negative counts. Posterior diagnostics confirmed that key parameters (r, K, and σ) and latent states were well-identified and converged reliably in JAGS.

Our 12-month ensemble forecast ultimately proved uninformative. The 95% CIs expanded so rapidly that they encompassed nearly all plausible tick-count values, yielding no clear directional trend. However, our uncertainty-partitioning analysis revealed that the forecast spread is substantially driven by parameter uncertainty in the growth rate and carrying capacity, with process noise adding only a short-term blip of variability and initial-condition uncertainty and simulated random effects remaining secondary.

Moving forward, incorporating environmental drivers (e.g., temperature, humidity) or including additional or new observations as they become available could further refine our forecast and reduce uncertainty.
=======
>>>>>>> ac200aaed0cf2f6e480ac1e902ba6aa520fb43a5
