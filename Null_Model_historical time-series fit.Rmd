...
tau_t ~ dgamma(1, 0.1)       #gamma prior on precision (shape 1, rate 0.1)

S ~ dgamma(0.1,0.1)   #parameter of precision
tau <- 1 / (pow(S, 2)) #define tau with S, part of process model

#data model
for(i in 1:n){
 count[i] ~ dnorm(Ex[i],tau)   #tick count as a normal distribution with mean Ex[i] defined below
 Ex[i] <- mu + alpha_t[month[i]]   #Ex[i] as a function of global mean plus random month effect
 }
 
 for(t in 1:nt){
  alpha_t[t] ~ dnorm(0,tau_t)		## random month effect (part of process model)
}
}
"
...
tall.null.jags <- jags.model(file = textConnection(TALL_null),
                      data = TALL_list,
                      n.chains = 3)
#remove burn in at 1700
burn_in <- 1700
update(tall.null.jags, n.iter = burn_in)
